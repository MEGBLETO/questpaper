  ...
  dbconn.query(sql, (err, result) => {
    if (err) {
      ...
      if (result.length === 0) {
        ...
        dbconn.query("INSERT INTO users (user_sirname, user_name, user_email, user_password, user_level) VALUES (?,?,?,?,?);" ,[sirname, name, email ,hash, year], (err, result) => {
          if (err) {
            console.log(err);
            throw err;
          }
...
router.post("/login", (req, res) => {
  ...
  dbconn.query("SELECT user_id, user_email, user_sirname, user_name,  user_password FROM users WHERE user_email = ?;", email, (error, result) => {
    if (error) {
      throw err;
    } else {
      if (result.length > 0) {
        ...
        bcrypt.compare(`${password}`, hash, function (error, response) {
          if (error) {
            ...
            var token = jwt.sign({auth: true,   data: result[0]}, "temp", {expiresIn: "1h" });
            console.log(token)
        ...
        res.status(401).send({message: "You do not have an account please create one"})
      }
    }
  });
...
router.post('/upload', (req, res) =>{
  ...
  file.mv(`${__dirname}/frontend/public/uploads/${file.name}`, function(err) {
    if (err){
      console.log(err)
